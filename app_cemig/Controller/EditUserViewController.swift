...
class EditUserViewController: UIViewController {
                        ...
                        if let data = data {
                            DispatchQueue.main.async {
                                do {
                                    ...
                                    self.nameTextField.text = user.nome
                                    self.cpfTextField.text = String(user.cpf)
                                    self.dateTextField.text = user.data_nasc
                                    self.cepTextField.text = String(user.cep)
                                    self.ruaTextField.text = user.rua
                                    self.numberTextField.text = String(user.number)
                                    self.bairroTextField.text = user.bairro
    ...
    func setDadosInAPI(){
        ...
        let url = ("https://apicemig.azurewebsites.net/api/usuario/email/"+defaults.string(forKey: "EmailDefaults")!)
        Alamofire.request(url, method:.put, parameters:parametersUser, encoding: URLEncoding.default).responseJSON { response in
            ...
            case .failure(_):
                let view = UIAlertController(title: "Ops! Algo deu errado", message: "Registro não pode ser Atualizado. Verifique as informações e valide novamente!", preferredStyle: .alert)
                let ok = UIAlertAction(title: "OK", style: .default, handler: {(_ action: UIAlertAction?) -> Void in
                    //Do some thing here
                    view.dismiss(animated: true) {() -> Void in }
                })
